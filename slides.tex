...
\begin{frame}
  ...
  e.g. a read function that only works on \texttt{`Opened ReadHandle}

  \alert{Foiled again!}

  The OCaml is too smart, it knows that \texttt{`Opened ReadHandle} is the
  same type as \texttt{`New ReadHandle} therefore every function which takes
  the \texttt{`Opened} handle takes every other type too
\end{frame}

\begin{frame}{Enter phantoms}
\end{frame}
